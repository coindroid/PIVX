...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zqncObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zqncObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zqncObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zQNCsupply", zqncObj));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zQNCsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zQNC denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zQNC denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zQNC denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zQNC denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zQNC denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zQNC denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zQNC denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zQNC denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zQNC denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in qnodecoin\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZQNC_FAILED_ACCUMULATOR_INITIALIZATION);
