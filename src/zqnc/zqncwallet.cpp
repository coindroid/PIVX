...
CzQNCWallet::CzQNCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zqnc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZQNCSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZQNCSeed_deprecated()) {
                    ...
                    LogPrintf("%s: failed to remove old zqnc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zqnc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
void CzQNCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zqncTracker->HasPubcoinHash(pMint.first)) {
...
bool CzQNCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    // Add to zqncTracker which also adds to database
    pwalletMain->zqncTracker->Add(dMint, true);
