...
CzQNCTracker::CzQNCTracker(std::string strWalletFile)
{
...
CzQNCTracker::~CzQNCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzQNCTracker::Init()
{
...
bool CzQNCTracker::Archive(CMintMeta& meta)
{
...
bool CzQNCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzQNCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzQNCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzQNCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzQNCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzQNCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzQNCTracker::GetSerialHashes()
{
...
CAmount CzQNCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzQNCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzQNCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzQNCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzQNCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzQNCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzQNCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzQNCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzQNCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzQNCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzQNCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzQNCWallet* zQNCWallet)
{
    bool iszQNCWalletInitialized = (NULL != zQNCWallet);
    CMintMeta meta;
    ...
    if (! iszQNCWalletInitialized)
        zQNCWallet = new CzQNCWallet(strWalletFile);
    meta.isSeedCorrect = zQNCWallet->CheckSeed(dMint);
    if (! iszQNCWalletInitialized)
        delete zQNCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzQNCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzQNCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzQNCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzQNCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzQNCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzQNCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzQNCWallet* zQNCWallet = new CzQNCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zQNCWallet);
        }
        delete zQNCWallet;
        LogPrint("zero", "%s: added %d dzqnc from DB\n", __func__, listDeterministicDB.size());
    }
