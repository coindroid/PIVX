...
CzQNCWallet::CzQNCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpivx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZQNCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZQNCSeed_deprecated()) {
                    LogPrintf("%s: Updated zQNC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpivx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpivx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzQNCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZQNCCount(nCountLastUsed);
    else if (!walletdb.ReadZQNCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzQNCWallet::Lock()
{
    seedMaster = 0;
}

void CzQNCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzQNCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZQNC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzQNCWallet::LoadMintPoolFromDB()
{
...
void CzQNCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzQNCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzQNCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpivxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzQNCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZQNC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpivxTracker which also adds to database
    pwalletMain->zpivxTracker->Add(dMint, true);
...
void CzQNCWallet::SeedToZQNC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzQNCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzQNCWallet::GenerateDeterministicZQNC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzQNCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzQNCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzQNCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
