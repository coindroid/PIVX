...
// The zQNC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zQNC is a hash of the serial
...
{
    //Create an output returning the zQNC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZQNCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zQNC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zQNC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZQNCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zQNC output", __func__);
...
{
    CzQNCTracker* zqncTracker = pwallet->zqncTracker.get();
    CMintMeta meta;
    if (!zqncTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zqncTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a QNC stake is the outpoint
