...
{
    CzQNCTracker* zpivxTracker = pwallet->zpivxTracker.get();
    CMintMeta meta;
    if (!zpivxTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpivxTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
