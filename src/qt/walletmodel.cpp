...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered qnodecoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 QNC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzQNCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzQNCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzQNCReset.connect(boost::bind(NotifyzQNCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzQNCReset.disconnect(boost::bind(NotifyzQNCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zqncTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
