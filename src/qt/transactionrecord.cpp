...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZQNCModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zpivxTracker->HasMintTx(hash))) {
            //zQNC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZQNC;
...
bool IsZQNCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZQNC:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZQNC || type == TransactionRecord::MNReward) {
