...
- [Development guidelines](#development-guidelines)
    - [General QnodeCoin Core](#general-qnodecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
QnodeCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play QNCs (tQNC)" on the test network, if you
...
QnodeCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/qnodecoin-project/qnodecoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_qnodecoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_qnodecoin --log_level=test_suite
$ valgrind -v --leak-check=full src/qnodecoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_qnodecoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates QNCs (if wallet is enabled).
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:QnodeCoin-Project/QnodeCoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `qnodecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `qnodecoin-cli` and the GUI debug console use this table to determine how to
